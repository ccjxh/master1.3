...
- (void)showToast:(UIView *)toast duration:(CGFloat)interval position:(id)point finishBlock:(finishBlock)block{
    toast.center = [self centerPointForPosition:point withToast:toast];
    toast.alpha = 0.0;
    [self addSubview:toast];
...
- (void)makeToast:(NSString *)message duration:(CGFloat)interval position:(id)position Finish:(finishBlock)block
{
    UIView *toast = [self viewForMessage:message title:nil image:nil];
    [self showToast:toast duration:interval position:position finishBlock:block];
